...
type fusestreamMasterScraper = (playerHTML: string) => (string | null | Promise<string|null>);
...
export class B64Scraper implements MasterScraper {
  public scrapers: fusestreamMasterScraper[] = [];

  constructor() {
    this.scrapers = [
			this.scrapeMasterM3U8V9
    ];
  }
  ...
  public async scrape(playerHTML: string): Promise<string|null> {
    ...
    for (let i = 0; i < this.scrapers.length; i++) {
      ...
      if (out)
		...
		// Cas de base
		console.error("no fusestream scraper was able to find the url");
    return null;
  }

  private async scrapeMasterM3U8V7(playerHTML: string): Promise<string|null> {
		const videojsURLRegExp = /https:\/\/fusevideo\.io\/(\w\/)*u\?(\w+=\w+&?)+/;
		...
		if (!(videojsURLMatch && videojsURLMatch[0])) {
			console.error("can't find fusestream's player js code in the player source");
		...
		if (!videojsRequest) {
			console.error("Fusevideo player js source request failed");
		...
		if (!videojsRaw) {
			console.error("Fusevideo videojs response is null");
  ...
  private async scrapeMasterM3U8V9(playerHTML: string): Promise<string|null> {
		const videojsURLRegExp = /https:\/\/fusevideo\.io\/(\w\/)*u\?(\w+=\w+&?)+/;
		const videojsURLMatch = playerHTML.match(videojsURLRegExp);
		...
		const b64Matches = videojsRaw.match(/atob\("[A-Za-z\d\/=]+"\)/);
		if (!(b64Matches && b64Matches[0])) {
			console.error("can't match b64 m3u8 source");
			console.error(`b64Matches: ${b64Matches}`);
			return null;
		...
		// Remove the parasite chars used to match the b64 scring
		const B64RawMatch = b64Matches[0].replace(/\(|\)|(atob)/g, "");
		console.log("got the b64 match: ", B64RawMatch);

		// Just recreated the process of the script himself, they added a random char at pos 1, removing it
		const B64decoded = decodeB64(B64RawMatch).slice(29).replace(/\|/g, "");
		try {
			const json = JSON.parse(B64decoded);
			console.log("url:", json.ezofpjbzoiefhzofsdhvuzehfg);
      return json.ezofpjbzoiefhzofsdhvuzehfg;
    } catch (err) {
      console.error(err);
			return null;
    }
